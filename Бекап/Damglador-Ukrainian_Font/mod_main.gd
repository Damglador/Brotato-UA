...
func _init(_modLoader = ModLoader):
	ModLoaderUtils.log_info("Init", UKRFONT)
	replace_fonts_preload()


func _ready()->void:
	ModLoaderUtils.log_info("Ready", UKRFONT)
...
func replace_fonts_preload() -> void:
	...
	for font_res in font_resources:
		...
		ModLoaderUtils.log_info("Replaced font: " + font_res.resource_path, UKRFONT)

		font_res.extra_spacing_bottom = -5
		font_res.extra_spacing_top = -3